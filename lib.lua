...
-- library init
	...
	writefile("ffff.ttf", game:HttpGet("https://github.com/boom1k/Allison-Murder-Mystery-2-Script-FREE/raw/refs/heads/main/tahoma.ttf"))
	...
	library.cloud_icon = ""
	do
		local CLOUD_ICON_B64 = ""
...
-- library functions 
	-- misc functions
		...
		-- DPI / UI scale. Drops a UIScale on every top-level frame of every
		-- library ScreenGui and keeps it updated; frames created later are
		-- caught via ChildAdded. clamp 0.5x .. 3x.
		function library:set_dpi(scale)
			library.dpi = math.clamp(tonumber(scale) or 1, 0.5, 3)
			library._dpi_hooked = library._dpi_hooked or {}

			local function scale_child(child)
				...
				ui.Scale = library.dpi
			end

			for _, gui in library.guis do
				if gui == library.cache then continue end
				for _, child in gui:GetChildren() do
					scale_child(child)
				end
				if not library._dpi_hooked[gui] then
					library._dpi_hooked[gui] = true
					library:connection(gui.ChildAdded, function(child)
						task.defer(scale_child, child)
					end)
				end
			end
	...
	-- elements 
		...
		function library:window(properties)
			...
			-- cfg holder
				...
				local ui_section = column:section({name = "Interface"})
					ui_section:slider({
						name = "DPI Scale", flag = "MENU_DPI",
						min = 0.5, max = 3, default = 1, interval = 0.05, suffix = "x",
						callback = function(value) library:set_dpi(value) end,
					})
			--
			...
			library:set_dpi(library.dpi) -- catch panels created above
		...
		function library:esp_preview(properties)
			...
			local source = lp.Character or lp.CharacterAdded:Wait()
			source.Archivable = true
			local character = source:Clone()
			source.Archivable = false

			local _animate = character:FindFirstChild("Animate")
			if _animate then _animate:Destroy() end

			-- a cloned rig usually loses PrimaryPart; PivotTo/SetPrimaryPartCFrame need it
			...
			if _humanoid then
				...
				_humanoid:ChangeState(Enum.HumanoidStateType.Physics)
			end

			local items = cfg.items; do
				-- wrapper is the list item; viewport + GUI overlay stack inside it
				-- (GUI children of a ViewportFrame do NOT render, so the ESP overlay
				--  has to be a sibling drawn on top of the viewport)
				...
				library:connection(run.RenderStepped, function()
					if not (character.Parent and character.PrimaryPart) then return end
					cfg.rotation += 0.5
					character:PivotTo(cfr(Vector3.new(0, 1, -6)) * angle(0, math.rad(cfg.rotation), 0))
				end)
			end
			...
			function cfg.refresh_elements( )
				objects.holder.Parent = flags["Enabled"] and items.overlay or library.cache
